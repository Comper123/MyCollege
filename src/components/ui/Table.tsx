      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-gray-200 dark:border-white/10">
            {columns.map((col, i) => (
              <th key={i} className="text-left px-4 py-3 font-medium text-gray-500 dark:text-white/50 whitespace-nowrap">
                {col.title}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {data.map((row, idx) => (
            <tr key={keyExtractor(row)} className="border-b border-gray-100 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
              {columns.map((col, i) => (
                <td key={i} className="px-4 py-3 text-gray-900 dark:text-white whitespace-nowrap">
                  {col.render 
